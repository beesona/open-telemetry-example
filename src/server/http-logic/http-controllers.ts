...
/**
 * Note about this class: it isnt used in this example but I want to keep the
 * source code for something else.
 */
export class HttpController<T> implements HttpController<T> {
    ...
    postRequest(req: HttpRequest): HttpResponse {
        const traceId =
            req.context.requestHeaders?.find((h) => h.key === 'TRACE_ID')?.value || '-1';
